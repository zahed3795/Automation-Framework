...
def show_options():
    ...
    print(sc)
    print("")
    line = "Here are some crypto pytest options to use with Super Framework:"
    line = c1 + line + cr
    print(line)
    print("")
...
def main():
    ...
    if command == "install":
        if len(command_args) >= 1:
            from framework.commands import super_install
            super_install.main()
        else:
            show_basic_usage()
            show_install_usage()
        ...
        if len(command_args) >= 0:
            from framework.crypto import obfuscate
            obfuscate.main()
        else:
            show_basic_usage()
            show_encrypt_usage()
        ...
        if len(command_args) >= 0:
            from framework.crypto import unobfuscate
            unobfuscate.main()
        else:
            show_basic_usage()
            show_decrypt_usage()
